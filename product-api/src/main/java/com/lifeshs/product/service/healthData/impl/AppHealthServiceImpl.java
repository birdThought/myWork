...
import com.lifeshs.product.common.constants.healthData.*;
...
import com.lifeshs.product.common.constants.jsonAttribute.base.Terminal;
import com.lifeshs.product.common.constants.jsonAttribute.healthData.BloodLipid;
import com.lifeshs.product.common.constants.jsonAttribute.healthData.Ecg;
import com.lifeshs.product.common.constants.jsonAttribute.healthData.Ua;
import com.lifeshs.product.common.constants.jsonAttribute.healthData.Uran;
import com.lifeshs.product.common.constants.jsonAttribute.healthData.*;
...
import com.lifeshs.product.common.conversion.Conversion;
import com.lifeshs.product.common.exception.base.OperationException;
import com.lifeshs.product.dao.user.IUserInfoReadDao;
import com.lifeshs.product.dao.user.IUserRecordDao;
...
import com.lifeshs.product.domain.dto.common.aop.AppJSONDataDTO;
import com.lifeshs.product.domain.dto.healthData.*;
import com.lifeshs.product.domain.dto.user.MemberUserDTO;
import com.lifeshs.product.domain.po.data.TDataHealthDescribe;
...
import com.lifeshs.product.domain.vo.user.UserInfoRead;
import com.lifeshs.product.service.healthData.IAppHealthService;
...
import com.lifeshs.product.utils.NumberUtils;
...
public class AppHealthServiceImpl extends AppNormalServiceImpl implements IAppHealthService {

    private static Logger logger = Logger.getLogger(AppHealthServiceImpl.class);

    @Autowired
    IUserInfoReadDao userInfoReadDao;
    ...
    public JSONObject addHeartRate(String json) throws Exception {
        ...
        for (int i = 0; i < mm.size(); i++) {
             ...
             */
            saveToUserInfoRead(userId, measureDate, DeviceType.HeartRate.value());
        }

        Integer healthProduct = appJSON.getAopData().getUser().getHealthProduct();
        bindHealthPackage(userId, healthProduct, DeviceType.HeartRate);
    ...
    public JSONObject addTemperature(String json) throws Exception {
        ...
        Integer healthProduct = appJSON.getAopData().getUser().getHealthProduct();
        bindHealthPackage(userId, healthProduct, DeviceType.Temperature);
        return success();
    }
    ...
    public JSONObject addBloodPressure(String json) throws OperationException {
        ...
        List<String> reasons = reasons(DeviceType.BloodPressure, params);
        ...
        Integer healthProduct = appJSON.getAopData().getUser().getHealthProduct();
        bindHealthPackage(userId, healthProduct, DeviceType.BloodPressure);
         ...
         *  每当用户上传测量数据,都需要插入或更新t_userInfo_read这张表
         */
        saveToUserInfoRead(userId, measureDate, DeviceType.BloodPressure.value());
...
//        throw new OperationException("测试异常抛出OperationException.class", 404);
    ...
    public JSONObject addOxygen(String json) throws Exception {
        ...
        params.add(getReasonSimpleDTO(heartRateStatus, HealthType.heartRate));
        List<String> reasons = reasons(DeviceType.Oxygen, params);
        ...
        Integer healthProduct = appJSON.getAopData().getUser().getHealthProduct();
        bindHealthPackage(userId, healthProduct, DeviceType.Oxygen);
         ...
         *  每当用户上传测量数据,都需要插入或更新t_userInfo_read这张表
         */
        saveToUserInfoRead(userId, measureDate, DeviceType.Oxygen.value());
    ...
    public JSONObject addGlucometer(String json) throws Exception {
        ...
        List<String> reasons = reasons(DeviceType.Glucometer, params);
        ...
        Integer healthProduct = appJSON.getAopData().getUser().getHealthProduct();
        bindHealthPackage(userId, healthProduct, DeviceType.Glucometer);
         ...
         *  每当用户上传测量数据,都需要插入或更新t_userInfo_read这张表
         */
        saveToUserInfoRead(userId, measureDate, DeviceType.Glucometer.value());
    ...
    public JSONObject addLunginstrument(String json) throws Exception {
        ...
        List<String> reasons = reasons(DeviceType.Lunginstrument, params);
        ...
        Integer healthProduct = appJSON.getAopData().getUser().getHealthProduct();
        bindHealthPackage(userId, healthProduct, DeviceType.Lunginstrument);
         ...
         *  每当用户上传测量数据,都需要插入或更新t_userInfo_read这张表
         */
        saveToUserInfoRead(userId, measureDate, DeviceType.Lunginstrument.value());
    ...
    public JSONObject addBodyfatscale(String json) throws Exception {
        ...
        List<String> reasons = reasons(DeviceType.BodyFatScale, params);
        ...
        Integer healthProduct = appJSON.getAopData().getUser().getHealthProduct();
        bindHealthPackage(userId, healthProduct, DeviceType.BodyFatScale);
         ...
         *  每当用户上传测量数据,都需要插入或更新t_userInfo_read这张表
         */
        saveToUserInfoRead(userId, measureDate, DeviceType.BodyFatScale.value());
    ...
    public JSONObject addBand(String json) throws Exception {
        ...
        for (int i = 0; i < mm.size(); i++) {
             ...
             *  每当用户上传测量数据,都需要插入或更新t_userInfo_read这张表
             */
            saveToUserInfoRead(userId, date, DeviceType.Band.value());
        }

        Integer healthProduct = appJSON.getAopData().getUser().getHealthProduct();
        bindHealthPackage(userId, healthProduct, DeviceType.Band);
     ...
     */
    private List<String> reasons(DeviceType packageType, List<ReasonSimpleDTO> params) {
    ...
    public JSONObject addEcg(String json) throws OperationException {
        ...
        for (int i = 0; i < mm.size(); i++) {
            JSONObject mm_i = mm.getJSONObject(i);
            int heartRate = mm_i.getIntValue(com.lifeshs.product.common.constants.jsonAttribute.healthData.Ecg.HEART_RATE);
            String measureDate = mm_i.getString(HealthPackage.MEASUREDATE);
            String image = mm_i.getString(com.lifeshs.product.common.constants.jsonAttribute.healthData.Ecg.IMAGE);
            Integer signType = mm_i.getInteger(com.lifeshs.product.common.constants.jsonAttribute.healthData.Ecg.SIGN_TYPE);
            String tags = mm_i.getString(com.lifeshs.product.common.constants.jsonAttribute.healthData.Ecg.TAGS);
        ...
        Integer healthProduct = appJSON.getAopData().getUser().getHealthProduct();
        bindHealthPackage(userId, healthProduct, DeviceType.ECG);
         ...
         *  每当用户上传测量数据,都需要插入或更新t_userInfo_read这张表
         */
        saveToUserInfoRead(userId, DateTimeUtilT.date(date), DeviceType.ECG.value());
    ...
    public JSONObject getEcg(String json) {
        ...
        if (!withDate.equals("0")) {
            ...
            for (EcgHistoryDTO h : hs) {
                ...
                Map<String, Object> returnData = new HashMap<>();
                returnData.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Ecg.DATE, h.getMeasureDate());
                returnData.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Ecg.AUTO_SIZE, autoSize);
                returnData.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Ecg.ACTIVE_SIZE, activeSize);
            ...
            for (EcgHistoryDTO h : hs) {
                for (EcgDTO e : h.getDatas()) {
                    ...
                    Map<String, Object> returnData = new HashMap<>();
                    returnData.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Ecg.DATE, DateTimeUtilT.dateTime(measureDate));
                    returnData.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Ecg.AUTO_SIZE, autoSize);
                    returnData.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Ecg.ACTIVE_SIZE, activeSize);
                    returnDatas.add(returnData);
                }
    ...
    public JSONObject getEcgWithDate(String json) {
        ...
        String withDate = mm_0.getString(com.lifeshs.product.common.constants.jsonAttribute.healthData.Ecg.DATE);
        String terminalType = mm_0.getString(Terminal.TYPE);
        String deviceType = Conversion.getDeviceType(terminalType);
        if ("-1".equals(deviceType)) {
            return success(NormalMessage.TERMINAL_NO_FOUND);
        ...
        if (hs.size() > 0) {
            ...
            returnData.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Ecg.AUTO_RECORD, ecgAutoMarkCount(hs_0));
            returnData.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Ecg.ACTIVE_RECORD, ecgActiveMarkCount(hs_0));
        }
        ...
        for (EcgDTO ecgDTO : ecgDTOs) {
            for (EcgDetailDTO ed : ecgDTO.getDetailList()) {
                if (0 == ed.getSignType().intValue()) {
                    ...
                    if (ecgDivideWithRhythmId.containsKey(rhythmId)) {
                        ecgMap = ecgDivideWithRhythmId.get(rhythmId);
                        records = (List<Map<String, Object>>) ecgMap.get(com.lifeshs.product.common.constants.jsonAttribute.healthData.Ecg.RECORD);
                    } else {
                        EcgRhythmDTO rhythmDTO = ed.getEcgRhythm();
                        ecgMap.put(Rhythm.NAME, rhythmDTO.getName());
                    ...
                    record.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Ecg.IMAGE, ed.getImage());
                    record.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Ecg.HEART_RATE, ed.getHeartRate());
                    record.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Ecg.MEASURE_DATE, ed.getMeasureDate());
                    records.add(record);
                    ecgMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Ecg.RECORD, records);
    ...
    private List<Map<String, Object>> ecgActiveMark(List<EcgDTO> ecgDTOs) {
        ...
        for (EcgDTO ecgDTO : ecgDTOs) {
            for (EcgDetailDTO ed : ecgDTO.getDetailList()) {
                if (1 == ed.getSignType().intValue()) {
                    Map<String, Object> map = new HashMap<>();
                    map.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Ecg.HEART_RATE, ed.getHeartRate());
                    map.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Ecg.MEASURE_DATE, ed.getMeasureDate());
                    map.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Ecg.TAGS, ed.getTagList());
                    map.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Ecg.IMAGE, ed.getImage());
                    list.add(map);
                }
            }
        }
    ...
    public JSONObject addUran(String json) throws OperationException {
        ...
        String deviceType = appJson.getType();
        JSONObject mm_0 = appJson.getData().getFirstJSONObject();
        String leu = mm_0.getString(com.lifeshs.product.common.constants.jsonAttribute.healthData.Uran.LEU);
        String nit = mm_0.getString(com.lifeshs.product.common.constants.jsonAttribute.healthData.Uran.NIT);
        String ubg = mm_0.getString(com.lifeshs.product.common.constants.jsonAttribute.healthData.Uran.UBG);
        String pro = mm_0.getString(com.lifeshs.product.common.constants.jsonAttribute.healthData.Uran.PRO);
        Float pH = mm_0.getFloat(com.lifeshs.product.common.constants.jsonAttribute.healthData.Uran.PH);
        String bld = mm_0.getString(com.lifeshs.product.common.constants.jsonAttribute.healthData.Uran.BLD);
        Float sg = mm_0.getFloat(com.lifeshs.product.common.constants.jsonAttribute.healthData.Uran.SG);
        String ket = mm_0.getString(com.lifeshs.product.common.constants.jsonAttribute.healthData.Uran.KET);
        String bil = mm_0.getString(com.lifeshs.product.common.constants.jsonAttribute.healthData.Uran.BIL);
        String glu = mm_0.getString(com.lifeshs.product.common.constants.jsonAttribute.healthData.Uran.GLU);
        String vc = mm_0.getString(com.lifeshs.product.common.constants.jsonAttribute.healthData.Uran.VC);
        String measureDate = mm_0.getString(HealthPackage.MEASUREDATE);
        String manualEntry = mm_0.getString(HealthPackage.MANUAL_ENTRY);
        Integer dataType = DataTypeEnum.REAL_TIME.value();
        if (manualEntry != null && Normal.TRUE.equals(manualEntry)) {
        ...
        List<String> reasons = reasons(DeviceType.URAN, params);
        ...
        Integer healthProduct = appJson.getAopData().getUser().getHealthProduct();
        bindHealthPackage(userId, healthProduct, DeviceType.URAN);
         ...
         *  每当用户上传测量数据,都需要插入或更新t_userInfo_read这张表
         */
        saveToUserInfoRead(userId, measureDate, DeviceType.URAN.value());
    ...
    public JSONObject getUran(String json) {
        ...
        for (UranDTO uranDTO : uranDTOList) {
            ...
            uranMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Uran.BIL, uranDTO.getBIL());
            uranMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Uran.BLD, uranDTO.getBLD());
            uranMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Uran.GLU, uranDTO.getGLU());
            uranMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Uran.KET, uranDTO.getKET());
            uranMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Uran.LEU, uranDTO.getLEU());
            uranMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Uran.NIT, uranDTO.getNIT());
            uranMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Uran.PH, uranDTO.getPH());
            uranMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Uran.PRO, uranDTO.getPRO());
            uranMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Uran.SG, NumberUtils.format(uranDTO.getSG(), 3));
            uranMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Uran.UBG, uranDTO.getUBG());
            uranMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Uran.VC, uranDTO.getVC());
            uranMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Uran.BIL_STATUS, uranDTO.getBILStatus());
            uranMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Uran.BLD_STATUS, uranDTO.getBLDStatus());
            uranMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Uran.GLU_STATUS, uranDTO.getGLUStatus());
            uranMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Uran.KET_STATUS, uranDTO.getKETStatus());
            uranMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Uran.LEU_STATUS, uranDTO.getLEUStatus());
            uranMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Uran.NIT_STATUS, uranDTO.getNITStatus());
            uranMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Uran.PH_STATUS, uranDTO.getPhStatus());
            uranMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Uran.PRO_STATUS, uranDTO.getPROStatus());
            uranMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Uran.SG_STATUS, uranDTO.getSgStatus());
            uranMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Uran.UBG_STATUS, uranDTO.getUBGStatus());
            uranMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Uran.VC_STATUS, uranDTO.getVCStatus());
            ...
            uranMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Uran.PH_AREA, uranDTO.getPHArea());
            uranMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Uran.SG_AREA, uranDTO.getSGArea());
            uranMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Uran.MEASURE_DATE, uranDTO.getMeasureDate());
    ...
    public JSONObject addUa(String json) throws OperationException {
        ...
        Float ua = mm_0.getFloat(com.lifeshs.product.common.constants.jsonAttribute.healthData.Ua.UA);
        String measureDate = mm_0.getString(HealthPackage.MEASUREDATE);
        String manualEntry = mm_0.getString(HealthPackage.MANUAL_ENTRY);
        Integer dataType = DataTypeEnum.REAL_TIME.value();
        if (manualEntry != null && Normal.TRUE.equals(manualEntry)) {
        ...
        Integer healthProduct = appJson.getAopData().getUser().getHealthProduct();
        bindHealthPackage(userId, healthProduct, DeviceType.UA);
         ...
         *  每当用户上传测量数据,都需要插入或更新t_userInfo_read这张表
         */
        saveToUserInfoRead(userId, measureDate, DeviceType.UA.value());
    ...
    public JSONObject getUa(String json) {
        ...
        for (UaDTO uaDTO : uaDTOList) {
            ...
            uaMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Ua.UA, uaDTO.getUA());
            uaMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Ua.UA_AREA, uaDTO.getUAArea());
            uaMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Ua.MEASURE_DATE, uaDTO.getMeasureDate());
            uaMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.Ua.STATUS, uaDTO.getStatus());
    ...
    public JSONObject addBloodLipid(String json) throws OperationException {
        ...
        Float hdl = mm_0.getFloat(com.lifeshs.product.common.constants.jsonAttribute.healthData.BloodLipid.HDL);
        Float ldl = mm_0.getFloat(com.lifeshs.product.common.constants.jsonAttribute.healthData.BloodLipid.LDL);
        Float tc = mm_0.getFloat(com.lifeshs.product.common.constants.jsonAttribute.healthData.BloodLipid.TC);
        Float tg = mm_0.getFloat(com.lifeshs.product.common.constants.jsonAttribute.healthData.BloodLipid.TG);
        Float lapidRatio = mm_0.getFloat(com.lifeshs.product.common.constants.jsonAttribute.healthData.BloodLipid.LAPID_RATIO);
        ...
        List<String> reasons = reasons(DeviceType.BloodLipid, params);
        ...
        Integer healthProduct = appJson.getAopData().getUser().getHealthProduct();
        bindHealthPackage(userId, healthProduct, DeviceType.BloodLipid);
         ...
         *  每当用户上传测量数据,都需要插入或更新t_userInfo_read这张表
         */
        saveToUserInfoRead(userId, measureDate, DeviceType.BloodLipid.value());
    ...
    public JSONObject getBloodLipid(String json) {
        ...
        for (BloodLipidDTO bloodLipidDTO : bloodLipidDTOList) {
            ...
            bloodLipidMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.BloodLipid.HDL, bloodLipidDTO.getHDL());
            bloodLipidMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.BloodLipid.HDL_AREA, bloodLipidDTO.getHDLArea());
            bloodLipidMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.BloodLipid.LDL, bloodLipidDTO.getLDL());
            bloodLipidMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.BloodLipid.LDL_AREA, bloodLipidDTO.getLDLArea());
            bloodLipidMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.BloodLipid.TC, bloodLipidDTO.getTC());
            bloodLipidMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.BloodLipid.TC_AREA, bloodLipidDTO.getTCArea());
            bloodLipidMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.BloodLipid.TG, bloodLipidDTO.getTG());
            bloodLipidMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.BloodLipid.TG_AREA, bloodLipidDTO.getTGArea());
            bloodLipidMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.BloodLipid.MEASURE_DATE, bloodLipidDTO.getMeasureDate());
            bloodLipidMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.BloodLipid.STATUS, bloodLipidDTO.getStatus());
            bloodLipidMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.BloodLipid.HDL_STATUS, bloodLipidDTO.getHDLStatus());
            bloodLipidMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.BloodLipid.LDL_STATUS, bloodLipidDTO.getLDLStatus());
            bloodLipidMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.BloodLipid.TC_STATUS, bloodLipidDTO.getTCStatus());
            bloodLipidMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.BloodLipid.TG_STATUS, bloodLipidDTO.getTGStatus());
            bloodLipidMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.BloodLipid.LAPID_RATIO, bloodLipidDTO.getBloodLipidRatio());
            bloodLipidMap.put(com.lifeshs.product.common.constants.jsonAttribute.healthData.BloodLipid.LIPID_RATIO_AREA, bloodLipidDTO.getBloodLipidRatioArea());
    ...
    public JSONObject getEcgAutoDataWithDate(String json) {
        ...
        JSONObject mm_0 = appJson.getData().getFirstJSONObject();
        String date = mm_0.getString(com.lifeshs.product.common.constants.jsonAttribute.healthData.Ecg.DATE);
        ...
        if (StringUtils.isBlank(date) || rhythmId == null || pageSize == null || pageIndex == null) {
            logger.error(ErrorInfo.PARAMETER_MISSING + ": date=" + date + "&rhythmId=" + rhythmId
                    + "&pageSize=" + pageSize + "&pageIndex=" + pageIndex);
            return error(ErrorInfo.PARAMETER_MISSING);
        }
    ...
    public JSONObject getEcgActiveDataWithDate(String json) {
        ...
        if (StringUtils.isBlank(date) || pageSize == null || pageIndex == null) {
            logger.error(ErrorInfo.PARAMETER_MISSING + ": date=" + date
                    + "&pageSize=" + pageSize + "&pageIndex=" + pageIndex);
            return error(ErrorInfo.PARAMETER_MISSING);
        }
    ...
    private void bindHealthPackage(int userId, Integer userHealthProduct, DeviceType healthProduct) {
        // 避免空值
