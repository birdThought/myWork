package com.lifeshs.product.service.record.impl;
...
import com.lifeshs.product.common.exception.database.DataBaseException;
import com.lifeshs.product.dao.healthData.*;
import com.lifeshs.product.dao.record.IRecordDao;
import com.lifeshs.product.dao.user.IMemberDao1;
...
import com.lifeshs.product.domain.dto.record.DietDTO;
...
public class RecordServiceImpl implements IRecordService {
    ...
    @Autowired
    private ISportBandDao sportBandDao;
    ...
    @Autowired
    private IEcgDao ecgDao;

    @Autowired
    private ITemperatureDao temperatureDao;

    @Autowired
    private IUranDao uranDao;

    @Autowired
    private IUaDao uaDao;

    @Autowired
    private IBloodLipidDao bloodLipidDao;
    ...
    public Map<String, Integer> queryDatesWithData(Integer userId, String queryDate, Integer queryType) throws DataBaseException {
        ...
        switch (queryType) {
            case 1://健康数据
                ...
                //心率手环
                if ((DeviceType.HeartRate.value() & healthProduct) == DeviceType.HeartRate.value()) {
...
//                        int status = po.getStatus().intValue();
                ...
                //血压仪
                if ((DeviceType.BloodPressure.value() & healthProduct) == DeviceType.BloodPressure.value()) {
...
//                        int status = po.getStatus().intValue();
                ...
                //血氧仪
                if ((DeviceType.Oxygen.value() & healthProduct) == DeviceType.Oxygen.value()) {
...
//                        int status = po.getStatus().intValue();
                ...
                //体脂称
                if ((DeviceType.BodyFatScale.value() & healthProduct) == DeviceType.BodyFatScale.value()) {
...
//                        int status = po.getStatus().intValue();
                ...
                //手环
                if ((DeviceType.Band.value() & healthProduct) == DeviceType.Band.value()) {
                ...
                //血糖仪
                if ((DeviceType.Glucometer.value() & healthProduct) == DeviceType.Glucometer.value()) {
...
//                        int status = po.getStatus().intValue();
                ...
                //肺活仪
                if ((DeviceType.Lunginstrument.value() & healthProduct) == DeviceType.Lunginstrument.value()) {
...
//                        int status = po.getStatus().intValue();
                ...
                //心电
                if ((DeviceType.ECG.value() & healthProduct) == DeviceType.ECG.value()) {
                ...
                //体温计
                if ((DeviceType.Temperature.value() & healthProduct) == DeviceType.Temperature.value()) {
...
//                        int status = po.getStatus().intValue();
                ...
                if ((DeviceType.BandSleep.value() & healthProduct) == DeviceType.BandSleep.value()) {
                    // System.out.println("睡眠");
                }

                //记步    记步的数据整合到了SportBand
                if ((DeviceType.BandStep.value() & healthProduct) == DeviceType.BandStep.value()) {
                    //System.out.println("记步");
                }

                //尿液分析仪
                if ((DeviceType.URAN.value() & healthProduct) == DeviceType.URAN.value()) {
...
//                        int status = po.getStatus().intValue();
                ...
                //尿检分析仪
                if ((DeviceType.UA.value() & healthProduct) == DeviceType.UA.value()) {
...
//                        int status = po.getStatus().intValue();
                ...
                //血脂分析仪
                if ((DeviceType.BloodLipid.value() & healthProduct) == DeviceType.BloodLipid.value()) {
